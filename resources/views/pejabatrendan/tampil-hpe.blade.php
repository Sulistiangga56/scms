...
<div class="card-body">
    ...
    <div id="alasan-form" style="display: none;">
        <form action="{{ route('pejabatrendan.approve-hpe', ['ID_Pengadaan' => $pengadaan->ID_Pengadaan,'ID_HPE' => $hpe->ID_HPE]) }}" method="POST">
            @csrf
            <div class="my-3 me-3">
                <label for="alasan_hpe">Alasan Setuju:</label>
            </div>
            <textarea name="alasan_hpe" id="alasan_hpe" cols="40" rows="5" style="border-radius: 5px"required></textarea><br>
            <button type="submit" class="btn btn-success my-2">Setuju</button>
        </form>
    </div>

    <div id="alasan-tolak-form" style="display: none;">
        <form action="{{ route('pejabatrendan.reject-hpe', ['ID_Pengadaan' => $pengadaan->ID_Pengadaan,'ID_HPE' => $hpe->ID_HPE]) }}" method="POST">
            @csrf
            <div class="my-3 me-3">
                <label for="alasan_hpe">Alasan Tolak:</label>
            </div>
            <textarea name="alasan_hpe" id="alasan_hpe" cols="40" rows="5" style="border-radius: 5px" required></textarea><br>
