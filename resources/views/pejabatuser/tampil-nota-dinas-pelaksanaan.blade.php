...
<div class="card-body">
    ...
    <div id="alasan-form" style="display: none;">
        <form action="{{ route('pejabatuser.approve-nota-dinas-pelaksanaan', ['ID_Pengadaan' => $pengadaan->ID_Pengadaan,'id_nota_dinas_permintaan' => $notaDinasPelaksanaan->id_nota_dinas_permintaan]) }}" method="POST">
            @csrf
            <div class="my-3 me-3">
                <label for="alasan_nota_dinas_pelaksanaan">Alasan Setuju:</label>
            </div>
            <textarea name="alasan_nota_dinas_pelaksanaan" id="alasan_nota_dinas_pelaksanaan" cols="40" rows="5" style="border-radius: 5px"required></textarea><br>
            <button type="submit" class="btn btn-success my-2">Setuju</button>
        </form>
    </div>

    <div id="alasan-tolak-form" style="display: none;">
        <form action="{{ route('pejabatuser.reject-nota-dinas-pelaksanaan', ['ID_Pengadaan' => $pengadaan->ID_Pengadaan,'id_nota_dinas_permintaan' => $notaDinasPelaksanaan->id_nota_dinas_permintaan]) }}" method="POST">
            @csrf
            <div class="my-3 me-3">
                <label for="alasan_nota_dinas_pelaksanaan">Alasan Tolak:</label>
            </div>
            <textarea name="alasan_nota_dinas_pelaksanaan" id="alasan_nota_dinas_pelaksanaan" cols="40" rows="5" style="border-radius: 5px" required></textarea><br>
