...
<div class="card-body">
    ...
    <td class="d-flex my-4">
        @if($pengadaan->id_status_ringkasan_rks === 4)
    ...
    <div id="alasan-form" style="display: none;">
        <form action="{{ route('pejabatuser.approve-rks', ['ID_Pengadaan' => $pengadaan->ID_Pengadaan,'ID_Ringkasan_Rks' => $rks->ID_Ringkasan_Rks]) }}" method="POST">
            @csrf
            <div class="my-3 me-3">
                <label for="alasan_rks">Alasan Setuju:</label>
            </div>
            <textarea name="alasan_rks" id="alasan_rks" cols="40" rows="5" style="border-radius: 5px"required></textarea><br>
            <button type="submit" class="btn btn-success my-2">Setuju</button>
        </form>
    </div>

    <div id="alasan-tolak-form" style="display: none;">
        <form action="{{ route('pejabatuser.reject-rks', ['ID_Pengadaan' => $pengadaan->ID_Pengadaan,'ID_Ringkasan_Rks' => $rks->ID_Ringkasan_Rks]) }}" method="POST">
            @csrf
            <div class="my-3 me-3">
                <label for="alasan_rks">Alasan Tolak:</label>
            </div>
            <textarea name="alasan_rks" id="alasan_rks" cols="40" rows="5" style="border-radius: 5px" required></textarea><br>
